...
| **AI-PQA** — AI power quality analyser (FastAPI + React) | `rdudr/AI-PQA` (working copy `C:\Users\risha\Desktop\ai-power-quality-analyzer`) | **Bundle** `PostMan-PQ-JSON v1` — `GET/POST /api/upload/session/{id}/postman.json` (`backend/reports/postman_bundle.py`): stats, series thinned to 240 points, harmonics, IEEE 519 / EN 50160 / IEC 61000 compliance rules, equipment health, events, data quality, observations — all computed on the server; `GET /api/upload/postman/sessions` lists what can be pulled. Fallback: *Export for PostMan* workbook `PostMan-PQ v1` (`postman_export.py`) | *Power quality analysis* under each main input / PCC / MCC panel (measured table, imbalance), then **Standards compliance** (score, rule table with the analyser's verdicts and remarks), **Equipment health** (five components, chart), **Events detected** (counts, worst events, dips/swells), data quality and observations; a **Compliance** column in the PCC load summary; the measurement charts in **Annexure A**; joined to the FOX panel by **Recording ID** | `src/p19_pq.js` — `PQ_COLS`, `pqMeta`, `importPq`, `pqLinkToPanels`, `recSummaryBlocks`, `recCharts`, `buildPqAnnexure`; `src/p19b_pqlink.js` — `importPqBundle`, `pqPullCard`, `pqComplianceBlocks`, `pqHealthBlocks`, `pqEventBlocks`, `pqScoreOf` | `docs/POSTMAN_EXPORT.md` in AI-PQA |
...
| **A-CMP** — air compressor assessment (Next.js, Android) | `rdudr/A-CMP` (working copy `C:\Users\risha\Desktop\A-CMP`) | sheet *Compressors* / *Compressor Entries*, columns = the app's field names (**the app does not write this export yet** — it makes a PDF; the importer is ready) | *Air compressor* chapter: name-plate, design vs actual SEC and air generation from the FAD / pump-up test | `src/p8_sld.js` — `importACmp`; `src/p7_modules.js` — `compressorCalc` | `docs/POSTMAN.md` in A-CMP |
...
| Compliance rules (IEEE 519 THD bands, EN 50160 ±10 % voltage and 50 Hz ±1 %, PF 0.95, IEC 61000-3-14 imbalance 2 %), health scoring, event detectors | AI-PQA `backend/reports/postman_bundle.py` — which must equal `frontend/src/utils/compliance.ts` and `equipmentHealth.ts` | PostMan prints the bundle's verdicts as received (`p19b_pqlink.js`); it does not recompute them |
