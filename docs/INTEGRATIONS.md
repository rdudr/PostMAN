...
| Compliance rules (IEEE 519 THD bands, EN 50160 ±10 % voltage and 50 Hz ±1 %, PF 0.95, IEC 61000-3-14 imbalance 2 %), health scoring, cost of poor quality, event detectors | AI-PQA `frontend/src/utils/compliance.ts`, `equipmentHealth.ts`, `costOfPoorQuality.ts` (the dashboard's figures travel with *Send to PostMan*); `backend/reports/postman_bundle.py` mirrors the first two for the file route | PostMan prints the bundle's figures as received (`p19b_pqlink.js`); it does not recompute them |
