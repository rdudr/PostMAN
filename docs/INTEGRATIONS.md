...
| **A-CMP** — air compressor assessment (Next.js, Android) | `rdudr/A-CMP` (working copy `C:\Users\risha\Desktop\A-CMP`) | Report → *Team Data Exchange (Excel)* → Export / Share (`lib/compressor-excel.ts`); the same workbook is attached to every emailed report. Sheets *Company Profile* (Field / Value, `Company Name`) and *Compressor Entries* (one row per compressor, columns = the app's field names, plus the derived `pumpTankVolumeM3`, `luLoadHours`, `luUnloadHours`, `luTotalHours`) | *Air compressor* chapter: name-plate, design vs actual SEC and air generation from the FAD / pump-up test, loaded-hours share | `src/p8_sld.js` — `isACmpWorkbook`, `importACmp`; `src/p7_modules.js` — `compressorCalc` | `docs/POSTMAN.md` in A-CMP |
...
| Compressor FAD: anemometer `area × velocity × 2118.88` CFM; pump-up `V(m³) × (P2 − P1) / (t/60 × 1.013)` m³/min (PostMan divides by 1.01325 — a 0.02 % difference, inside the rounding); design / actual SEC = kW / CFM; verdict: actual SEC more than 10 % above design is flagged | A-CMP `app/(console)/compressors/page.tsx` (screen) and `lib/pdf-generator.ts` (PDF) | PostMan `compressorCalc` in `p7_modules.js` |
