...
function importPqBundle(b, fileName){
  ...
  var rec = {
    ...
    quality: b.data_quality || null, observations: b.observations || [], bundle: true
  };
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
function pqServerUrl(){ pqState(); return String(S.pq.server || '').replace(/\/+$/, ''); }
...
function pqPullCard(w){
  ...
  row.appendChild(btn('List recordings', function(){
    ...
    pqListSessions().then(function(sessions){
      clear(list);
      if (!sessions.length){ show('The server has no recordings to offer. Process a file on the analyser first.', ''); return; }
      status.hidden = true;
      ...
      sessions.forEach(function(s){
        ...
        head.appendChild(el('strong','', (s.filename || 'session ' + s.session_id.slice(0, 8)) + (s.plant_name ? ' — ' + s.plant_name : '')));
        head.appendChild(el('span','font-size:11px;color:var(--ink-3);', [s.company_name, s.analyzer, s.audit_date, s.total_rows ? inr(s.total_rows) + ' rows' : '', s.in_memory ? 'in memory' : 'stored'].filter(Boolean).join(' · ')));
        ...
        var role = el('select'); [['main','Plant main input'],['pcc','PCC panel'],['mcc','MCC panel']].forEach(function(o){ var op = el('option','', o[1]); op.value = o[0]; role.appendChild(op); }); role.value = 'pcc';
        var panel = el('input'); panel.type = 'text'; panel.placeholder = 'Panel name';
        var rid = el('input'); rid.type = 'text'; rid.placeholder = 'Recording ID';
        ...
        g.appendChild(labelled('Panel from FOX', sel)); g.appendChild(labelled('Measured at', role)); g.appendChild(labelled('Panel name', panel)); g.appendChild(labelled('Recording ID', rid));
        box.appendChild(g);
      ...
      var go = btn('Import selected recordings', function(){
        ...
        var next = function(){
          ...
          pqPullSession(p.s.session_id, p.role.value, p.panel.value.trim(), p.rid.value.trim())
            .then(function(b){ try { lines.push(['ok', importPqBundle(b)]); } catch (e){ lines.push(['bad', e.message]); } next(); })
  ...
  c.appendChild(el('p','margin-top:8px;font-size:12px;color:var(--ink-3);', 'Offline instead: on the analyser dashboard, Export for PostMan → Download PostMan bundle (JSON), then drop the .json file in the box above.'));
  w.appendChild(c);
}
