...
/* Standards compliance, laid out as the analyser's own audit PDF lays it
   out: a headline band, then one table per standard with what it checks,
   how many of its clauses pass, and each clause's measured value, limit,
   verdict and the analyser's remark. */
function pqComplianceBlocks(B, rec){
  ...
  B.push(blk(bP('Evaluated by the AI power quality analyser against IEEE 519, EN 50160, IEC 61000-3-14 and utility power-factor norms over the whole recording (' + inr(rec.samples) + ' samples). A marginal result counts as half a pass in the score.', { size:9.5 })));
  B.push(blk(bKPI([
    ['Overall compliance', (sm.score !== undefined ? sm.score : '—') + ' %'], ['Checks', String(sm.total || cmp.rules.length)],
    ['Pass', String(sm.pass || 0)], ['Marginal', String(sm.warn || 0)], ['Fail', String(sm.fail || 0)]
  ])));
  /* group by standard, in the analyser's order */
  var groups = [], byStd = {};
  cmp.rules.forEach(function(r){ if (!byStd[r.standard]){ byStd[r.standard] = []; groups.push(r.standard); } byStd[r.standard].push(r); });
  groups.forEach(function(std){
    var items = byStd[std], passN = items.filter(function(r){ return r.verdict === 'pass'; }).length;
    var descs = []; items.forEach(function(r){ if (r.description && descs.indexOf(r.description) < 0) descs.push(r.description); });
    B.push(tblBlock(null, [[{ v:std, tone:'head' }, { v:passN + ' / ' + items.length + ' pass', tone:(passN === items.length ? 'ok' : items.some(function(r){ return r.verdict === 'fail'; }) ? 'bad' : 'watch') }]], { colw:['70%','30%'] }));
    if (descs.length) B.push(blk(bP(descs.join(' '), { size:9, italic:true })));
    B.push(tblBlock(['Clause', 'Measured', 'Limit', 'Verdict', 'Remark'], items.map(function(r){
      var d = r.unit === '' ? 3 : 2;
      return [r.clause, fix(r.measured, d) + (r.unit ? ' ' + r.unit : ''), fix(r.limit, r.unit === '' ? 2 : 0) + (r.unit ? ' ' + r.unit : ''),
        { v:PQ_VERDICT[r.verdict] || r.verdict, tone:PQ_TONE[r.verdict] || null }, r.remark || ''];
    }), { size:8, colw:['22%','14%','12%','11%','41%'] }));
  });
  /* the picture: verdicts per standard */
  B.push(blk(bChart(chartStacked(groups.map(function(g){ return g.replace(' (typical)', ''); }), [
    { name:'Pass', color:C.ok, values:groups.map(function(g){ return byStd[g].filter(function(r){ return r.verdict === 'pass'; }).length; }) },
    { name:'Marginal', color:C.watch, values:groups.map(function(g){ return byStd[g].filter(function(r){ return r.verdict === 'warn'; }).length; }) },
    { name:'Fail', color:C.bad, values:groups.map(function(g){ return byStd[g].filter(function(r){ return r.verdict === 'fail'; }).length; }) }
  ], 'Compliance verdicts by standard', 'checks'), 'Number of clauses passing, marginal and failing under each standard.')));
}

/* Equipment health, as in the analyser's audit PDF: the weighted score,
   the five components with what was measured, their weight, score and
   status, the score chart, and the recommended actions. */
function pqHealthBlocks(B, rec){
  var h = rec.health; if (!h || !h.components) return;
  var status = String(h.status || (h.overall >= 80 ? 'good' : h.overall >= 55 ? 'fair' : 'poor'));
  B.push(blk(bH(3, 'Equipment health — ' + (rec.recId || rec.name))));
  B.push(blk(bP('Weighted composite of harmonics, voltage stability, power factor, three-phase balance and frequency, scored 0–100 by the analyser (good ≥ 80, fair ≥ 55, poor below).', { size:9.5 })));
  B.push(blk(bKPI([['Overall health', h.overall + ' / 100 — ' + status.replace(/^./, function(x){ return x.toUpperCase(); })]].concat(h.components.map(function(c){ return [c.label, c.score + ' (' + (c.status || '') + ')']; })))));
  B.push(tblBlock(['Component', 'Measured', 'Weight', 'Score', 'Status', 'Finding'],
    h.components.map(function(c){ return [c.label, fix(c.raw, 3), Math.round((c.weight || 0) * 100) + ' %', String(c.score),
      { v:String(c.status || '').replace(/^./, function(x){ return x.toUpperCase(); }), tone:PQ_TONE[c.status] || null }, c.detail || '']; }),
    { size:8, colw:['17%','10%','8%','7%','9%','49%'] }));
  B.push(blk(bChart(chartBarsRef(h.components.map(function(c){ return c.label.replace(/ (Health|Stability|Balance)$/, ''); }), h.components.map(function(c){ return c.score; }),
    'Equipment health scores', 'score / 100', { ref:{ value:80, label:'Good ≥ 80' }, colorAt:function(v){ return v >= 80 ? C.ok : v >= 55 ? C.watch : C.bad; } }))));
  B.push(tblBlock(['Recommended action', 'For'], h.components.map(function(c){ return [c.recommendation || '—', c.label]; }), { size:8.5, colw:['76%','24%'] }));
}

/* Cost of poor power quality — the analyser's Cost page arithmetic, run
   here on the rates in force for this report. The analyser's tariff
   figures arrive with the recording; PostMan's own rates (bill analysis,
   cost register) can replace them on the Electrical distribution page,
   and the printed section says which were used. */
function pqCostCalc(rec, i){
  var s = rec.stats || {};
  var avgKw = s.kw ? s.kw.avg : 0, avgKva = s.kva ? s.kva.avg : 0, avgPf = s.pf ? s.pf.avg : 1, vthd = s.vthd ? s.vthd.avg : 0;
  var annualHours = tn(i.hoursPerDay) * tn(i.daysPerYear), annualKwh = avgKw * annualHours;
  var energyCost = annualKwh * tn(i.tariff), demandCost = avgKva * tn(i.demandCharge) * 12;
  var pfDeficit = Math.max(0, tn(i.pfThreshold) - avgPf), pfPenaltyPctTotal = Math.round(pfDeficit * 100) * tn(i.pfPenaltyPct);
  var pfPenalty = (energyCost + demandCost) * (pfPenaltyPctTotal / 100);
  var harmonicLossPct = vthd * 0.5, harmonicLoss = energyCost * (harmonicLossPct / 100);
  var targetPf = Math.max(0.95, tn(i.pfThreshold)), newKva = avgPf > 0 ? avgKw / targetPf : avgKva;
  var kvaSavings = Math.max(0, (avgKva - newKva) * tn(i.demandCharge) * 12);
  var totalLeak = pfPenalty + harmonicLoss;
  return { annualHours:annualHours, annualKwh:annualKwh, energyCost:energyCost, demandCost:demandCost, avgKw:avgKw, avgKva:avgKva, avgPf:avgPf, vthd:vthd,
           pfDeficit:pfDeficit, pfPenaltyPctTotal:pfPenaltyPctTotal, pfPenalty:pfPenalty, harmonicLossPct:harmonicLossPct, harmonicLoss:harmonicLoss,
           targetPf:targetPf, newKva:newKva, kvaSavings:kvaSavings, totalLeak:totalLeak, totalPotential:totalLeak + kvaSavings };
}
/* The rates this report already knows: the bills' blended rate and the
   DISCOM demand slab, the cost register's unit rate and working days. */
function pqReportRates(){
  var out = {};
  try { var c = billCalc(); if (c && c.avg && c.avg.rate) out.tariff = +Number(c.avg.rate).toFixed(2); } catch (e){}
  if (!out.tariff && num(S.costs.unitRate) !== null) out.tariff = num(S.costs.unitRate);
  try { var cfg = billCfg(); if (cfg && cfg.slab1Rate) out.demandCharge = cfg.slab1Rate; } catch (e){}
  if (num(S.costs.days) !== null) out.daysPerYear = num(S.costs.days);
  return out;
}
function pqCostInputs(rec){
  var base = (rec.cost && rec.cost.inputs) || { tariff:9.5, demandCharge:350, pfThreshold:0.95, pfPenaltyPct:1, hoursPerDay:16, daysPerYear:300, currency:'₹' };
  if (!rec.costInputs) rec.costInputs = { tariff:base.tariff, demandCharge:base.demandCharge, pfThreshold:base.pfThreshold, pfPenaltyPct:base.pfPenaltyPct, hoursPerDay:base.hoursPerDay, daysPerYear:base.daysPerYear, currency:base.currency || '₹', source:'analyser' };
  return rec.costInputs;
}
function pqCostBlocks(B, rec){
  if (!rec.cost && !(rec.stats && rec.stats.kw)) return;
  var i = pqCostInputs(rec), r = pqCostCalc(rec, i), cur = i.currency || '₹';
  ...
  B.push(blk(bP('The recording’s averages translated into annual rupees: the utility’s low power-factor penalty, the extra losses that harmonics cause in transformers and cables, and the billed kVA that power-factor correction would remove. Rule-of-thumb figures for scale; the bill analysis chapter carries the tariff-exact working.', { size:9.5 })));
  B.push(blk(bKPI([
    ['Annual leakage', money(r.totalLeak)], ['PF penalty', money(r.pfPenalty)], ['Harmonic losses', money(r.harmonicLoss)], ['kVA saving if PF corrected', money(r.kvaSavings)], ['Total recoverable', money(r.totalPotential)]
  ])));
  ...
  B.push(blk(bChart(chartBarsRef(['PF penalty', 'Harmonic', 'kVA saving', 'Total'], [r.pfPenalty, r.harmonicLoss, r.kvaSavings, r.totalPotential], 'Cost of poor power quality', cur + ' per year',
    { colorAt:function(v, k){ return k === 3 ? C.navy : k === 2 ? C.ok : C.bad; } }), 'Annual penalty and losses now, the demand saving available, and the total recoverable.')));
  B.push(blk(bP('Tariff figures ' + (i.source === 'postman' ? 'as set in PostMan from this report’s bill analysis and cost register' : 'as entered on the analyser’s Cost page') + ': ' + cur + ' ' + i.tariff + '/kWh, ' + cur + ' ' + i.demandCharge + '/kVA/month, ' + i.hoursPerDay + ' h/day, ' + i.daysPerYear + ' days/year, penalty ' + i.pfPenaltyPct + ' % per 0.01 below PF ' + i.pfThreshold + '.', { size:9, italic:true })));
}

/* The tariff card for a recording on the Electrical distribution page. */
function pqCostCard(rec){
  var i = pqCostInputs(rec);
  var box = el('div','border-top:1px dashed var(--line);margin-top:8px;padding-top:8px;');
  var head = el('div','display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:6px;');
  head.appendChild(el('strong','font-size:12px;', 'Cost of poor quality — tariff figures'));
  head.appendChild(el('span','font-size:11px;color:var(--ink-3);', i.source === 'postman' ? 'set in PostMan' : 'as entered on the analyser'));
  var use = btn('Use this report’s rates', function(){
    var rr = pqReportRates(), n = 0;
    Object.keys(rr).forEach(function(k){ if (rr[k] !== undefined && rr[k] !== null){ i[k] = rr[k]; n++; } });
    if (!n){ alert('No rates yet: read the bills (blended ₹/kWh and the demand slab) or set the unit rate and working days in the Cost register first.'); return; }
    i.source = 'postman'; save(); renderAll();
  }, 'sm'); use.style.marginLeft = 'auto'; head.appendChild(use);
  if (rec.cost && rec.cost.inputs) head.appendChild(btn('Back to analyser’s', function(){ rec.costInputs = null; save(); renderAll(); }, 'sm'));
  box.appendChild(head);
  var mark = function(f){ var inp = f.querySelector('input'); if (inp) inp.addEventListener('input', function(){ i.source = 'postman'; }); return f; };
  box.appendChild(gridOf([
    mark(fNum(i,'tariff','Tariff ₹/kWh')), mark(fNum(i,'demandCharge','Demand charge ₹/kVA/month')), mark(fNum(i,'pfThreshold','PF threshold')),
    mark(fNum(i,'pfPenaltyPct','Penalty % per 0.01 PF')), mark(fNum(i,'hoursPerDay','Hours per day')), mark(fNum(i,'daysPerYear','Days per year'))
  ]));
  var r = pqCostCalc(rec, i);
  box.appendChild(el('div','font-size:12px;color:var(--ink-2);margin-top:6px;font-family:"IBM Plex Mono",monospace;',
    'leakage ₹ ' + inr(Math.round(r.totalLeak)) + ' /yr · recoverable ₹ ' + inr(Math.round(r.totalPotential)) + ' /yr'));
  return box;
}
...
recSummaryBlocks = function(B, rec){
  if (rec.charts && rec.charts.length){
    ...
    /* Every chart the analyser captured for this panel, here under it;
       the findings follow the graphs. */
    B.push(blk(bH(3, 'Measurement charts — ' + (rec.recId || rec.name))));
    pqChartBlocks(B, rec, /./);
...
function pqAnnexCharts(B, rec){
  _recChartsPlain(rec, 'vipfuch').forEach(function(c){ B.push(blk(bChart(c[1]))); });
}
/* A recording sent from the analyser prints its charts under its panel, so
   the annexure lists only recordings that arrived as a workbook. */
function pqInAnnexure(rec){ return !(rec.charts && rec.charts.length); }
